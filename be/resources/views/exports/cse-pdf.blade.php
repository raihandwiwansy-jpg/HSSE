...
<head>
    <meta charset="utf-8">
    <title>Laporan Confined Space Entry (CSE)</title>
    <style>
...
<body>
    ...
    <table style="width:100%; margin-bottom:3px;">
        <tr>
            <td align="center">
                <span style="font-size:12px; font-weight:bold; color:#1B5E20;">LAPORAN CONFINED SPACE ENTRY (CSE)</span>
            </td>
        </tr>
    </table>
    ...
    <table style="width:100%; font-size:7px; margin-bottom:1px;">
        <tr>
            <td style="width:40%;"><b>No. Dokumen:</b> FM-BSHS-19/02</td>
    ...
    <table style="width:100%; border:1px solid #2E7D32; font-size:7px;">
        <thead>
            <tr style="background-color:#2E7D32; color:#fff;">
                ...
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:9%;">Supervisor</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:10%;">Fasilitas</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:9%;">Lokasi</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:12%;">Alasan</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:6%;">Jml Pekerja</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:6%;">%LEL</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:6%;">%O2</th>
                <th style="border:1px solid #1B5E20; padding:3px 2px; text-align:center; width:7%;">PPM H2S</th>
        ...
        <tbody>
            ...
            <tr style="background-color:{{ $bg }};">
                ...
                <td style="border:1px solid #ccc;">{{ $item->supervisor ?? '-' }}</td>
                <td style="border:1px solid #ccc;">{{ $item->fasilitas ?? '-' }}</td>
                <td style="border:1px solid #ccc;">{{ $item->lokasi ?? '-' }}</td>
                <td style="border:1px solid #ccc;">{{ $item->alasan ?? '-' }}</td>
                <td style="border:1px solid #ccc; text-align:center;">{{ $item->jumlah_pekerja ?? '-' }}</td>
                <td style="border:1px solid #ccc; text-align:center; font-weight:bold;">
                    @php $lel = isset($item->hasil_gas) ? ($item->hasil_gas['lel'] ?? '-') : ($item->lel ?? '-'); @endphp
                    {{ $lel }}
                    @if(is_numeric($lel))
                        @if($lel < 10) <span style="color:#065f46;">(Aman)</span>
                        @else <span style="color:#991b1b;">(Bahaya)</span>
                        @endif
                    @endif
                </td>
                <td style="border:1px solid #ccc; text-align:center; font-weight:bold;">
                    @php $o2 = isset($item->hasil_gas) ? ($item->hasil_gas['o2'] ?? '-') : ($item->o2 ?? '-'); @endphp
                    {{ $o2 }}
                    @if(is_numeric($o2))
                        @if($o2 >= 19.5 && $o2 <= 23.5) <span style="color:#065f46;">(Normal)</span>
                        @else <span style="color:#991b1b;">(Abnormal)</span>
                        @endif
                    @endif
                </td>
                <td style="border:1px solid #ccc; text-align:center; font-weight:bold;">
                    @php $h2s = isset($item->hasil_gas) ? ($item->hasil_gas['h2s'] ?? '-') : ($item->h2s ?? '-'); @endphp
                    {{ $h2s }}
                    @if(is_numeric($h2s))
                        @if($h2s < 10) <span style="color:#065f46;">(Aman)</span>
                        @else <span style="color:#991b1b;">(Bahaya)</span>
                        @endif
                    @endif
                </td>
            ...
            @empty
            <tr>
                <td colspan="13" style="border:1px solid #ccc; text-align:center; padding:10px; color:#888;">Tidak ada data CSE</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <table style="width:100%; margin-top:4px; font-size:6px;">
        <tr>
            <td style="font-style:italic; color:#666;">
                <b>Note:</b> %LEL (&lt;10% aman), %O2 (19.5-23.5% normal), PPM H2S (&lt;10ppm aman) |
    ...
    <table style="width:100%; margin-top:2px; border-top:1px solid #ddd;">
        <tr>
            <td align="center" style="font-size:6px; color:#999; padding-top:2px;">
                Total: {{ count($data) }} data CSE | Dicetak otomatis oleh Sistem HSE - PT. Industri Nabati Lestari
            </td>
        </tr>
    </table>
</body>

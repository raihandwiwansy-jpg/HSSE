...
class LwpExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    protected $status;
    protected $dateFrom;
    protected $dateTo;
    ...
    public function title(): string { return 'Laporan LWP'; }
    ...
    {
        return ['No', 'Tanggal', 'Lokasi', 'Pekerjaan', 'Beban Total', 'Kapasitas Crane', 'Status', 'Catatan', 'Pemohon'];
    }

    public function map($lwp): array
    {
        $this->rowNumber++;
        return [
            $this->rowNumber,
            $lwp->tanggal ? \Carbon\Carbon::parse($lwp->tanggal)->format('d/m/Y') : '-',
            $lwp->lokasi ?? '-',
            $lwp->pekerjaan ?? '-',
            $lwp->beban_total ?? '-',
            $lwp->kapasitas_crane ?? '-',
            ucfirst($lwp->status ?? 'Draft'),
            $lwp->catatan ?? '-',
            $lwp->user->name ?? '-',
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $sheet->setCellValue('A2', 'LAPORAN CRITICAL LIFTING WORK PERMIT (LWP)');
                $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12)->getColor()->setARGB('1B5E20');
                $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getRowDimension(2)->setRowHeight(25);
                $sheet->mergeCells('A3:' . $lastCol . '3');
                $sheet->setCellValue('A3', 'No. Dokumen: FM-BSHS-19/07 | No. Revisi: 00 | Tgl. Berlaku: 2024-08-01 | Tgl. Cetak: ' . now()->format('d/m/Y H:i'));

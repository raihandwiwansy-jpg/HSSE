...
class ElpExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    protected $status;
    protected $dateFrom;
    protected $dateTo;
    ...
    public function title(): string { return 'Laporan ELP'; }
    ...
    {
        return ['No', 'Tanggal', 'Shift', 'Alat Diisolasi', 'Tag No', 'Tipe Isolasi', 'Status', 'Catatan', 'Pemohon'];
    }

    public function map($elp): array
    {
        $this->rowNumber++;
        return [
            $this->rowNumber,
            $elp->tanggal ? \Carbon\Carbon::parse($elp->tanggal)->format('d/m/Y') : '-',
            $elp->shift ?? '-',
            $elp->alat_diisolasi ?? '-',
            $elp->tag_no ?? '-',
            $elp->tipe_isolasi ?? '-',
            ucfirst($elp->status ?? 'Draft'),
            $elp->catatan ?? '-',
            $elp->user->name ?? '-',
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $sheet->setCellValue('A2', 'LAPORAN ELECTRICAL WORK PERMIT (ELP)');
                $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12)->getColor()->setARGB('1B5E20');
                $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getRowDimension(2)->setRowHeight(25);
                $sheet->mergeCells('A3:' . $lastCol . '3');
                $sheet->setCellValue('A3', 'No. Dokumen: FM-BSHS-19/05 | No. Revisi: 00 | Tgl. Berlaku: 2024-08-01 | Tgl. Cetak: ' . now()->format('d/m/Y H:i'));

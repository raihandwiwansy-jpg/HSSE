...
class RwpExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    protected $status;
    protected $dateFrom;
    protected $dateTo;
    ...
    public function title(): string { return 'Laporan RWP'; }
    ...
    {
        return ['No', 'Tanggal', 'Lokasi', 'Peralatan', 'Area/Ruang', 'Tipe Sumber', 'Status', 'Catatan', 'Pemohon'];
    }

    public function map($rwp): array
    {
        $this->rowNumber++;
        return [
            $this->rowNumber,
            $rwp->tanggal ? \Carbon\Carbon::parse($rwp->tanggal)->format('d/m/Y') : '-',
            $rwp->lokasi ?? '-',
            $rwp->peralatan ?? '-',
            $rwp->area ?? '-',
            $rwp->tipe_sumber ?? '-',
            ucfirst($rwp->status ?? 'Draft'),
            $rwp->catatan ?? '-',
            $rwp->user->name ?? '-',
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $sheet->setCellValue('A2', 'LAPORAN RADIOGRAPHY WORK PERMIT (RWP)');
                $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12)->getColor()->setARGB('1B5E20');
                $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getRowDimension(2)->setRowHeight(25);
                $sheet->mergeCells('A3:' . $lastCol . '3');
                $sheet->setCellValue('A3', 'No. Dokumen: FM-BSHS-19/08 | No. Revisi: 00 | Tgl. Berlaku: 2024-08-01 | Tgl. Cetak: ' . now()->format('d/m/Y H:i'));

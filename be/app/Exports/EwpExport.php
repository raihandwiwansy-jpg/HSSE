...
class EwpExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    protected $status;
    protected $dateFrom;
    protected $dateTo;
    ...
    public function title(): string { return 'Laporan EWP'; }
    ...
    {
        return ['No', 'Tanggal', 'Lokasi', 'Panjang', 'Lebar', 'Kedalaman', 'Peralatan', 'Status', 'Catatan', 'Pemohon'];
    }

    public function map($ewp): array
    {
        $this->rowNumber++;
        return [
            $this->rowNumber,
            $ewp->tanggal ? \Carbon\Carbon::parse($ewp->tanggal)->format('d/m/Y') : '-',
            $ewp->lokasi ?? '-',
            $ewp->panjang ?? '-',
            $ewp->lebar ?? '-',
            $ewp->kedalaman ?? '-',
            $ewp->peralatan ?? '-',
            ucfirst($ewp->status ?? 'Draft'),
            $ewp->catatan ?? '-',
            $ewp->user->name ?? '-',
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $sheet->setCellValue('A2', 'LAPORAN EXCAVATION WORK PERMIT (EWP)');
                $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12)->getColor()->setARGB('1B5E20');
                $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getRowDimension(2)->setRowHeight(25);
                $sheet->mergeCells('A3:' . $lastCol . '3');
                $sheet->setCellValue('A3', 'No. Dokumen: FM-BSHS-19/06 | No. Revisi: 00 | Tgl. Berlaku: 2024-08-01 | Tgl. Cetak: ' . now()->format('d/m/Y H:i'));

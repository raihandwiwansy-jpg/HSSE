...
class InsidenExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    protected $status;
    protected $dateFrom;
    protected $dateTo;
    ...
    public function collection()
    {
        ...
        if ($this->dateFrom) {
            $query->where('tanggal_kejadian', '>=', $this->dateFrom);
        }
        if ($this->dateTo) {
            $query->where('tanggal_kejadian', '<=', $this->dateTo);
        }
    ...
    {
        return ['No', 'Judul', 'Jenis', 'Lokasi', 'Tanggal Kejadian', 'Deskripsi', 'Pelapor', 'Status'];
    }

    public function map($insiden): array
    {
        $this->rowNumber++;
        return [
            $this->rowNumber,
            $insiden->judul,
            ucfirst($insiden->jenis),
            $insiden->lokasi,
            \Carbon\Carbon::parse($insiden->tanggal_kejadian)->format('d/m/Y'),
            $insiden->deskripsi,
            $insiden->user->name ?? '-',
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $sheet->setCellValue('A2', 'LAPORAN INSIDEN');
                $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12)->getColor()->setARGB('1B5E20');
                $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getRowDimension(2)->setRowHeight(25);

                $sheet->mergeCells('A3:' . $lastCol . '3');
                $sheet->setCellValue('A3', 'No. Dokumen: FM-BSHS-19/INS | No. Revisi: 00 | Tgl. Cetak: ' . now()->format('d/m/Y H:i'));

...
import { Eye, Edit3, Send, Trash2, Calendar, MapPin } from 'lucide-react';
...
export default function GwpCard({ data, isOwner, isAdmin, onEdit, onsubmit, onDelete }: GwpCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm dark:bg-[#1E1E2E] dark:border-[#333355] transition-all hover:shadow-md">
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100">
            GWP-{String(data.id).padStart(3, '0')}
          </h3>
      ...
      <div className="space-y-2 mb-3">
        <p className="text-sm text-gray-700 dark:text-gray-300 font-medium">{data.departemen}</p>
      ...
      <div className="flex items-center gap-2 mb-3">
        <StatusBadge status={data.kategori_pekerjaan} />
        <StatusBadge status={data.kategori_risiko} />
      </div>

      {/* Actions */}
      <div className="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-gray-700">
        <button
          onClick={() => router.push(`/gwp/${data.id}`)}
